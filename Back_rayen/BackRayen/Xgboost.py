...
def train_xgboost(df, target_column):
    ...
    print(f"Training XGBoost model for {target_column}...")
    ...
    with mlflow.start_run(nested=True, run_name=f"XGBoost_{target_column}"):
        ...
        plt.legend()
        plt.savefig(f"xgboost_forecast_{target_column}.png")
        mlflow.log_artifact(f"xgboost_forecast_{target_column}.png")
        plt.close()
        
        forecast_df = pd.DataFrame({target_column: predictions}, index=y_test.index)
        forecast_df.to_csv(f"forecast_xgboost_{target_column}.csv", index=False)
        mlflow.log_artifact(f"forecast_xgboost_{target_column}.csv")
