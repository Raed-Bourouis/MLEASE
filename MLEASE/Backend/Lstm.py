...
def train_lstm(df, target_column):
    ...
    with mlflow.start_run(nested=True, run_name=f"LSTM_{target_column}"):
        ...
        plt.legend()
        plt.savefig(f"lstm_forecast_{target_column}.png")
        mlflow.log_artifact(f"lstm_forecast_{target_column}.png")
        plt.close()
        
        forecast_df = pd.DataFrame({target_column: y_pred_rescaled.flatten()}, index=y_test[seq_length:].index)
        forecast_df.to_csv(f"forecast_lstm_{target_column}.csv", index=False)
        mlflow.log_artifact(f"forecast_lstm_{target_column}.csv")
